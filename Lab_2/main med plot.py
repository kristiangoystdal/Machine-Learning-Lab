...
# Get the best model training using the best n, m and d values
X_train, Y_train, scaler = calculateXandY(n, m, d, p, N_y, y_train)
ridge.fit(X_train, Y_train)

# Combine x_train and x_test into a full dataset
x_full = np.concatenate((x_train, x_test), axis=0)
N_full = len(x_full)

# Prepare for predictions lists on x_test
X_test = []
y_test_pred = []
...
for i in range(N_y, N_full):
    ...
    X_test_scaled = scaler.transform([X_test[-1]])  # Scale the input
